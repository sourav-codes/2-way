...
import { defaultSystemMessage } from "./prompts.ts";
...
async function start_realtime(endpoint: string, apiKey: string, deploymentOrModel: string) {
  ...
  try {
    ...
    console.log(error);
    makeNewTextBlock("[Connection error]: Unable to send initial config message. Please check your endpoint and authentication details.", "info");
...
function createConfigMessage(): SessionUpdateMessage {
  ...
  if (systemMessage) configMessage.session.instructions = systemMessage;
  if (!isNaN(temperature)) configMessage.session.temperature = temperature;
  if (voice) configMessage.session.voice = voice;
  ...
  for await (const message of realtimeStreaming.messages()) {
    ...
    switch (message.type) {
      case "session.created":
        ...
        makeNewTextBlock(`**Session started**`, "info");
        makeNewTextBlock();
        break;
      ...
      case "conversation.item.input_audio_transcription.completed":
        ...
        // latestInputSpeechBlock.textContent += " You: " + message.transcript;
        break;
      case "response.done":
...
function getSystemMessage(): string {
  return formSessionInstructionsField.value || defaultSystemMessage
...
function makeNewTextBlock(text: string = "", className: string = "") {
...
formStartButton.addEventListener("click", async () => {
  ...
  const endpoint = 'https://zuhai-m2inntsr-eastus2.openai.azure.com/openai/realtime?api-version=2024-10-01-preview&deployment=gpt-4o-realtime-preview';
  const key = 'e07f76e1354947a391e43eae8b69ba32';
  ...
  try {
    formStartButton.classList.add("hidden");
    formStopButton.classList.remove("hidden")
...
formStopButton.addEventListener("click", async () => {
  formStopButton.classList.add("hidden")
  formStartButton.classList.remove("hidden");
  setFormInputState(InputState.Working);
  resetAudio(false);
